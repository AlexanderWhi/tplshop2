	...
	foreach ($catalog as $item) {
		...
		<div class="img_name_desc">
			...
			<?$img=($item['img']?$item['img']:$this->cfg('NO_IMG'))?>
                    <a class="img  <?if($this->cfg('SHOP_SCALE_IMAGE')=='true'){?>image<?}?>" title=" <?=Html::encode($item['name'])?>" style="background-image:url('<?=Img::scaleBySize($img,$item['unit'],array(218,170))?>')"  href="<?=$url?>" rel="<?=scaleImg($img,'w420h420')?>"></a>
