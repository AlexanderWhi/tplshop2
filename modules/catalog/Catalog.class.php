...
//			$condition.=" AND EXISTS (SELECT item_id FROM sc_shop_prop_val WHERE prop_id={$prop} AND i.id=item_id)";
		...
		if($prop=$get->getArray('p')){
			...
			foreach ($prop as $pid=>$val) {
				...
				if(is_array($val)){
					foreach ($val as &$v) {$v=SQL::slashes($v);}
...
//			}
	...
	function actFilter(){
		...
		header("Location: ".$this->getURI(array(),$query));exit;
	}
...
//			'default'=>'по&nbsp;умолчанию',
		...
		foreach ($names as $name=>$label){
			...
			}else{?><a class="ord_<?=$name?>" href="<?=$this->getURI(array('ord'=>$name,'sort'=>$sort_def),true)?>"><ico><span><?=$label?></span></ico></a><?}
		}	
	}
...
//		}
		...
		}elseif(in_array($this->getURIVal('catalog'),array('fog','manid','prop','action'))){
...
//			$this->displayGoods(true);
	...
	function actSearchGoods(){
		global $post;
		header("Location: ".$this->mod_uri.'?search='.urlencode($post->get('search')));exit;
	}
...
//			$mail_contacts=$this->enum('mail_contacts',$this->getRegion());
	...
	function actGifts(){
		...
		header("Location: /catalog/order/?d=2");exit;
	}
	...
	function actAdd(){
		...
		if($rs->next()){
			...
			if($rs->get('in_stock')==0){
				echo printJSON(array('error'=>'временно не продается'));exit;
			}
		}
...
//		setcookie('basket',$_COOKIE['basket']=json_encode($basket),COOKIE_EXP,'/');
		...
		echo $this->render(array(),dirname(__FILE__).'/basket_content_container.php');exit;
	}
	function actClear(){
		if(isset($_GET['id'])){
			$id=intval($_GET['id']);
...
//			}
			...
			if($post->exists('sort')){$d['sort']=$post->get('sort');}
			if($post->exists('ord')){$d['ord']=$post->get('ord');}
			
			
		}
		echo $this->renderBasketContent($d);exit;
	}
...
//				$att[]=array('name'=>'Счёт.xls','file'=>$url."&access=allow");
	...
	function actGetRenderBasket(){
		...
		echo $this->renderBasketContent($post->get());exit;
	}
...
//		}
	...
	function actRemove(){
		...
		unset($basket[$key]);		
		$this->saveBasketData($basket);
		echo printJSON(array('key'=>$key));exit;
	}
...
//			setcookie('sid',session_id(),time() + 3600*24*30,'/');
			echo printJSON($row);exit;
		}
		echo printJSON(array('msg'=>'Ошибка авторизации'));exit;
	}
...
//		if(!trim($args->get('fullname' ))){$error['fullname']="Введите ФИО!";}
		if(!trim($args->get('phone'))){$error['phone']='Введите телефон';}
		if(!trim($args->get('address'))){$error['address']='Введите адрес';}
...
//		}
	...
	function actSendOrder(){
		...
		if($error=$this->checkOrder($post,$basket)){
			echo printJSON(array('error'=>$error));exit;
		}else{
			...
			if(!$this->getUserId() && $post->getInt('reg')==1 /*$post->exists('reg_login') */){//&& $post->exists('want_reg')
				if($post->exists('mail')){
					...
					//уведомление о регистрации
		          	$this->sendTemplateMail($data['mail'],'notice_new_user',
		          		array('FROM_SITE'=>FROM_SITE,'LOGIN'=>$data['login'],'PASSWORD'=>$password)
		          	);
		          	//уведомление о регистрации админу
		          	$this->sendTemplateMail($this->cfg('MAIL'),'notice_new_user4admin',
		          		array('FROM_SITE'=>FROM_SITE,'LOGIN'=>$data['login'],'name'=>$data['name'])
		          	);
...
//				$att[]=array('name'=>'Счёт.xls','file'=>$url."&access=allow");
			...
			if($mail){
				$this->sendTemplateMail($post->get('mail'),'notice_new_order',
		         	$notice,
		         	$att
		       	);
			}
...
//			$mail_contacts=$this->enum('mail_contacts',$this->getRegion());
	       	$this->sendTemplateMail($this->cfg('MAIL'),'notice_new_order4admin',
	          	$notice
	       	);
...
//			setcookie('basket',null,0,'/');//Очистим корзину
			...
			echo printJSON(array(
				...
				'date'=>"{$post->get('date')} {$post->get('time')}"));exit;
		}
	}
	...
	function actFavRemove(){
		...
		header('Location: '.$_SERVER['HTTP_REFERER']);exit;
		echo printJSON(array('msg'=>'ok','id'=>$get->getInt('id')));exit;
	}
	function actAddFav(){
		global $post;
		$count=LibCatalog::addFav($post->getInt('id'));
		echo printJSON(array('count'=>$count));exit;
	}
	///////////////////
	// Сравнение
	////////////////////
