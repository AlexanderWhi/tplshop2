		...
		if($post->exists('all')){
			...
			if($rs->next()){
				if($rs->get('cache_child_catalog_ids')){
					...
					if($arr){
						$ST->update('sc_shop_catalog',
						array('extraid'=>$field['extraid']),"id IN(".join(',',$arr).")");
					}
				}
			}
		}
		echo printJSONP(array('msg'=>'Сохранено','id'=>$id)+$field);exit;
	}
...
//	}
	...
	function actSetDistributor(){
		...
		echo printJSON(array('msg'=>'Обновлено '.count($ids).' записей','ids'=>$ids));exit;
	}
	...
	function actUpload(){
		...
		echo printJSONP(array('msg'=>'Сохранено','paths'=>$paths,'imgs'=>$imgs),$get->get('cb'));exit;
	}
	...
	function actUpdateCatalogCache(){
		...
		echo printJSON(array('msg'=>'Применено!'));exit();
	}
...
//        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');	
	...
	function actSaveSimular(){
		...
		echo printJSON($post->getArray('item'));exit;
	}
	...
	function actRemoveRelation(){
		...
		echo printJSON(array('msg'=>"Удалено ".count($rel_item)." связей"));exit;
	}
    ...
    function actRemoveToCategory(){
        ...
        if (!empty($ids)) { 
        	...
        	if($get->get('mode')=='copy'){
	            ...
	            echo printJSON(array('msg'=>"Товары скопированы в категорию с индексом ".$category_id));exit;
        	}elseif($get->get('mode')=='remove'){
        		$ST->delete("sc_shop_item2cat","catid=$category_id AND itemid IN (".implode(',',$ids).")");
        		echo printJSON(array('msg'=>"Товары извлечены из категории с индексом ".$category_id));exit;
        	}else{
	        	foreach($ids as $id) {	
	                $ST->update("sc_shop_item", array('category'=>$category_id), 'id='.$id);
	            }
	            echo printJSON(array('msg'=>"Товары перемещены в категорию с индексом ".$category_id));exit;
        	}
        }
...
//            }
        ...
        echo printJSON(array('msg'=>"Для ".count($ids)." товаров обновлено ".(count($remove)+count($new_prop_name))." свойств"));exit;
    }
    /**
    ...
    function actSetGoodsMan(){
        ...
        echo printJSON(array('msg'=>"Для ".count($ids)." товаров обновлено "));exit;
    }
		...
		if($this->isAdmin() ){
			...
			if(!empty($sort1)){
				foreach ($sort1 as $id=>$val){
					...
					if(isset($sort[$id])){$d['sort']=$sort[$id];}
					if(isset($sort2[$id])){$d['sort2']=$sort2[$id];}
					if(isset($sort3[$id])){$d['sort3']=$sort3[$id];}
					if(isset($ext_id[$id])){$d['ext_id']=$ext_id[$id];}
					if(isset($in_stock[$id])){$d['in_stock']=$in_stock[$id];}
					if(isset($price[$id])){$d['price']=$price[$id];}
...
//			'id'=>$id,
		...
		echo printJSON(array('msg'=>'Сохранено','id'=>$id,'field'=>$field,'post'=>$post->get()));exit;
	}
	...
	function actDeleteGoods(){
		...
		echo printJSON(array('msg'=>'Удалено','ids'=>$ids));exit;
	}
...
//		return array(4=>'Список выбор');
	...
	function actApplyProps(){
		...
		echo printJSON(array('msg'=>'Обновлено!'));exit;
	}
	...
	function actPropToNum(){
		...
		echo printJSON(array('msg'=>"Обновлено $c!"));exit;
	}
...
//	}
	...
	function _displayCatalog($catalog,$selected,$deep=0,$n=0){
		...
		<?foreach($catalog as $item){?>
			...
			<?if(!empty($item['children'])){$this->_displayCatalog($item['children'],$selected,$deep+1,$n);	}?>
		<?}?>	
	<?}
...
//			if($i++==0){continue;}
		...
		echo printJSON(array('msg'=>'Импортировано'));exit;
		...
		echo printJSON(array('msg'=>'Импортировано за '.$t.' сек.'));exit;
	}
	...
	function actExport(){
        ...
        header('Expires: 0');
        header('Cache-Control: must-revalidate, post-check=0, pre-check=0');	
		echo $this->render($data,dirname(__FILE__).'/admin_catalog_export.tpl.php');exit;
	}
...
//		echo printJSON(array('msg'=>'Сохранено ','id'=>$id));exit;
	...
	function actApplyMans(){
		...
		if(isset($_FILES['img'])){
			...
			echo printJSONP(array('msg'=>'Обновлено!','img'=>$img));exit;
		}
