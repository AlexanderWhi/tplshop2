...
//			$ps[$rs->get('name')]->setEmail($this->getUser('mail'));
		...
		while ($rs->next()) {
			...
			$orderItem=array();
			$rs1=$ST->select("SELECT oi.*,i.name,i.img,c.img AS c_img ,i.description,i.category
...
//					'num'=>$num,
					'is_universal'=>$rs1->get('category')==0
				);
			}
...
//		$this->setCommonCont();
	...
	function actMakeBasket(){
		...
		while ($rs->next()) {
			...
			$this->addBasket($rs->get('itemid'),$rs->get('count'));
...
//		setcookie('basket',json_encode($res),COOKIE_EXP,'/');
	...
	function actMakeNote(){
		global $ST,$get;
		$noteid=$ST->insert('sc_shop_note',array('name'=>"Заказ ".$get->getInt('id'),'userid'=>$this->getUserId()));
