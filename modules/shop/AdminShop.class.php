...
include_once('modules/shop/ShopConfig.class.php');
...
//		$this->actDefault();
    ...
    function renderStatusList() {
        ...
        foreach ($order_status_list as $st => $desc) {
            $list[$st] = $desc;
            if (!empty($order_status_count[$st]))
                $list[$st].=" [$order_status_count[$st]]";
        }
...
//		$this->logData('saveOrder',$post);
        ...
        if ($post->get('country')) {
            $data['country'] = $post->get('country');
        }
        if ($post->get('region')) {
            $data['region'] = $post->get('region');
        }
        if ($post->get('city')) {
        ...
        if ($post->get('phone')) {
            $data['phone'] = $post->get('phone');
        }
        if ($post->get('additionally')) {
            $data['additionally'] = $post->get('additionally');
        }
        if ($post->exists('pay_status')) {
            $data['pay_status'] = $post->get('pay_status');
        }
        if ($post->exists('pay_system')) {
            $data['pay_system'] = $post->get('pay_system');
        }
