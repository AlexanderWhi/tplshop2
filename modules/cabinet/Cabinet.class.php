...
//          	);
			...
			echo printJSON(array('error'=>$this->error));exit;
		}
	}
	...
	function actActivateBonus(){
		...
		$summ=0;
			...
			if($rs->next()){
				if($rs->get('user')>0){
					...
					$this->sendTemplateMail($this->cfg('MAIL'),'notice_activate_bonus',
		         		array('id'=>$this->getUserId(),'login'=>$this->getUser('login'),'code'=>$post->get('num'),'pass'=>$post->get('pass'),)
		       		);
				}
		...
		echo printJSON(array('error'=>$error,'summ'=>$summ+$this->getUser('bonus')));exit;
	}
		...
		if($post->isMail('mail')){
	        ...
	        if(!$rs->next()){
	        	...
	        	if($t==array_unique(array_merge($t,$type))){
	        		echo printJSON(array('msg'=>'Уже подписаны'));exit;
	        	}
	        	$t=array_unique(array_merge($t,$type));
	        	$ST->update('sc_subscribe',array('type'=>implode(' ',$t)),"mail='".SQL::slashes($post->get('mail'))."'");
	        }
	        echo printJSON(array('msg'=>BaseComponent::getText('subscribe_confirm')));exit;
		}else{
			echo printJSON(array('err'=>'Введите адрес корректно!'));exit;
		}
	}
