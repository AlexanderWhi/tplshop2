...
class Join extends Component {
	...
	function actLogin(){
		...
		if($rs->next()){
			...
			if(preg_match('|/login|',$this->getURI())){
				if($this->isAdmin() || $this->getUser('type')=='operator'){
					if($post->exists('ref') && $post->get('ref')){
						header("Location: {$post->get('ref')}");exit;
					}
			...
			header('Location: /join/?error=true');exit;
...
//				$fields['refid']=intval($_SESSION['refid']);
//			}
...
//          	}
	...
	function actQSign(){
		...
		if($err=$this->qcheckAll($post)){
			echo printJSON(array('err'=>$err));exit;
		}else{
...
//          	$_SESSION['_USER']['u_id']=intval($id);
	        ...
	        $this->noticeICQ($this->cfg('ICQ'),'Новый пользователь на сайте');         	
			echo printJSON(array('msg'=>'Вы успешно зарегистрировались, логин пароль были отправлены на почту'));exit;
		}	
	}
