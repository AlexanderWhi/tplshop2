...
<td rowspan="3" class="img">
	...
	<?if($item['img']){?>
		<img src="<?=scaleImg($item['img'],'w190')?>">
	<?}?>
	</a>
</td>
...
<span class="date title" title="Годен до <?=dte($date)?>">
<?=dte($date)?>
...
<tr>
<?if(true){?>
<td style="vertical-align:top"><?if($item['c_img']){?><img src="<?=$item['c_img']?>"><?}?>Купленные сертификаты (<?=$item['count']?>):</td>
...
<?}?>
<tr>
	<td colspan="1">Итого: <strong class="price"><?=number_format($price,0,',',' ');?>&nbsp;р.</strong></td>
	<td></td>
	<td></td>
