...
//		}
		...
		$queryStr="SELECT o.*, u.name AS u_name,u.phone AS u_phone, u.type,d.value_desc AS delivery_type,p.value_desc AS pay_system,ps.value_desc AS pay_status FROM sc_shop_order AS o
			LEFT JOIN sc_users AS u ON u.u_id=o.userid
			LEFT JOIN (SELECT field_value,value_desc FROM sc_enum WHERE field_name='sh_delivery_type') AS d ON d.field_value=o.delivery_type
			LEFT JOIN (SELECT field_value,value_desc FROM sc_enum WHERE field_name='sh_pay_system') AS p ON p.field_value=o.pay_system
...
//		$this->actDefault();
	...
	function renderStatusList(){
		...
		return $this->render($data,'core/tpl/common/select.tpl.php');
	}
...
//				}
		...
		//Другие заказы
		$rs=$ST->select("SELECT * FROM sc_shop_order WHERE  userid=".$field['userid']." ORDER BY create_time");
...
//		}
	...
	function renderOrderContent($id,$d=array()){
		global $ST;
		$orderItem=array();
		$rs=$ST->select("SELECT oi.*,i.id AS i_id, i.name,i.img,i.description,nmn.description as nmn_description
			FROM 
			sc_shop_item AS i,
			sc_shop_order_item AS oi
			LEFT JOIN sc_shop_proposal AS p ON oi.proposalid=p.id 
			LEFT JOIN sc_shop_item_nmn AS nmn ON nmn.id=oi.nmn 
				 
			WHERE oi.itemid=i.id   AND oi.orderid=".$id);
		while ($rs->next()) {
...
//	}
	...
	function actPrint(){
		...
		$queryStr="SELECT *,o.*, u.mail,u.address AS u_address,u.mail AS u_mail,u.phone AS u_phone, ps.value_desc AS pay_system, pst.value_desc AS pay_status  FROM sc_shop_order AS o 
			LEFT JOIN sc_users AS u ON u.u_id=o.userid
		...
		while ($rs->next()) {
			...
			$data['rs'][$rs->getInt('id')]['orderContent']=$this->render(array('orderContent'=>$basket),dirname(__FILE__).'/ordercontent_print.tpl.php');	
		}
		
		$this->setTitle('Печать заказа');
		$this->tplContainer="core/tpl/admin/admin_print.php";
		$this->display($data,dirname(__FILE__).'/admin_order_print.tpl.php');
	}
