...
//		$this->updateCatalog();
    ...
    function needAuth() {
        if ($this->getUserId() == 0) {
            header("Location: /join/?rel={$this->getURI()}");
            exit;
        }
    }
        ...
        if (isset($this->advertising[$id])) {
            ...
            if (trim($advertising['file'])) {
                ...
                if (preg_match('/\.(jpg|jpeg|png|gif)$/i', $advertising['file'])) {
                    if ($href)
                        $r.='<a href="' . $href . '" style="padding:0" title="' . $advertising['description'] . '" >';
                    $r.='<img src="' . $advertising['file'] . '" alt="' . $advertising['url'] . '" style="margin:0;border:0">';
                    if ($href)
                        $r.='</a>';
                }elseif (preg_match('/\.(swf)$/i', $advertising['file'])) {
                    if ($href)
                        $r.='<a title="' . $advertising['description'] . '"  href="' . $href . '" style="display:block;position:absolute;z-index:1;width:100%;height:100%;background:url(/img/pixel.gif)"></a>';
