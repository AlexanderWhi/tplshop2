<form method="POST">
    <table class="form">
        <td><a href="<?= $this->getUri(array('usertype' => null)) ?>">Все</a>
            <? foreach ($usertype as $k => $ut) { ?>
                <a href="<?= $this->getUri(array('usertype' => $k)) ?>"><?= $ut ?></a> 
            <? } ?></td>
        <td style="text-align:right">
            <a href="?act=subscribe">Подписчики</a> [<a href="?act=mailDownload">Выгрузка адресов</a>]
        ...
        <table class="grid">
            ...
            <? while ($this->rs->next()) { ?>
                <tr>
                    ...
                    <? } else { ?>
                        <td><strong><a href="<?= $this->getUri(array('usertype' => $this->rs->get('type'))) ?>" title="Выбрать тип"><?= $this->rs->get('type'); ?></a></strong></td>
                    <? } ?>
                    ...
                    <td>
                        <small><?= htmlspecialchars(trim($this->rs->get("city") . ' ' . $this->rs->get("phone") . ' ' . $this->rs->get("mail"))); ?></small>	
                    </td>
