...
class HashMap {
    ...
    function isMail($key) {
        return isset($this->map[$key]) ? !(empty($this->map[$key]) || !check_mail($this->map[$key])) : null;
    }
     ...
     */
    function getInt($key) {
        return isset($this->map[$key]) ? intval($this->map[$key]) : 0;
    }
    ...
    function getFloat($key) {
        return isset($this->map[$key]) ? floatval(str_replace(",", ".", $this->map[$key])) : 0;
    }
     ...
     */
    function getNum($key) {
        return isset($this->map[$key]) ? preg_replace('/\D/', '', $this->map[$key]) : 0;
    }

    function getDate($key) {
        return isset($this->map[$key]) ? date('Y-m-d', strtotime($this->map[$key])) : null;
    }
